        ...
        public List<SearchedUser> FindUserByName(string userId, string searchText, double latitude, double longitude, bool isCurrentLocation)
        {
            ...
            searchResult = dtSearchedUser.AsEnumerable().Select(row =>
                                            new SearchedUser
                                            {
                                                ...
                                                // ShowProfileImage = (Convert.ToBoolean(row["showProfileImage"]) == false || IO.FileManager.GetFile(Convert.ToString(row["uid"]), FileCategory.Profile, MediaType.Image) == null) ? false : true
                                                ShowProfileImage = (FileManager.GetFile(Convert.ToString(row["uid"]), FileCategory.Profile, MediaType.Image) == null) ? false : true
