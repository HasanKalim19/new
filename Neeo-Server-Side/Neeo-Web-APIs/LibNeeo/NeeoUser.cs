        ...
        public List<ContactAvatarTimestamp> GetContactsAvatarTimestamp(string contacts)
        {
            ...
            if (contactsArray.Length > 0)
            {
                for (int i = 0; i < contactsArray.Length; i++)
                {
                    ...
                    AvatarState avatarState = contact.GetAvatarState(0, false, out avatarTimestamp, out fileInfo);
        ...
        public List<ContactStatus> GetContactsAvatarTimestamp(SyncData syncData)
        {
            ...
            if (syncData.ContactsList.Count > 0)
            {
                foreach (var contact in syncData.ContactsList)
                {
                    ...
                    user.GetAvatarState(0, false, out avatarTimestamp, out avatarInfo);
                    ...
                    if (getAll)
                    {
                        switch (dr["ContactRosterInfo"].ToString())
                        {
                            ...
                            case exists:
                                ...
                                contact.AvatarState = (ushort)contactUser.GetAvatarState(0, false, out avatarTimestamp, out avatarInfo);
