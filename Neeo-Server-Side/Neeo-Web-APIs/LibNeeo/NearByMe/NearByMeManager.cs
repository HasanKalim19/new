...
namespace LibNeeo.NearByMe
{
    public class NearByMeManager
    {
        ...
        public List<NearByUser> GetNearByUsersByLocation(UserSearch user)
        {
            ...
            nearByUsers = (from row in dtNearbyUser.AsEnumerable()
                           select new NearByUser()
                           {
                               ...
                               ShowProfileImage = (FileManager.GetFile(Convert.ToString(row["username"]), FileCategory.Profile, MediaType.Image) == null) ? false : true,
                               IsPrivateAccount = Convert.ToBoolean(row["isPrivateAccount"]),
                               Status = (FriendRequestStatus)Convert.ToInt32(row["status"])
                           }).ToList();
        ...
        public async Task<List<Countries>> GetNBMMembersCountryList()
        {
            List<Countries> countries = new List<Countries>();
            DataTable dbNBMMembersCountryList = await System.Threading.Tasks.Task.Run(() => _dbManager.GetNBMMembersCountryList());

            countries = (from row in dbNBMMembersCountryList.AsEnumerable()
