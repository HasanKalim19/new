...
namespace PowerfulPal.Neeo.NearByMeApi.Controllers
{
    ...
    public class NearByMeController : NeeoApiController
    {
        ...
        [HttpGet]
        [Route("GetNBMMembersCountryList")]
        public async Task<HttpResponseMessage> GetNBMMembersCountryList()
        {
            try
            {
                LogRequest("GetNBMMembersCountryList");
                if (!ModelState.IsValid)
                {
                    return Request.CreateErrorResponse(HttpStatusCode.BadRequest, ModelState);
                }

                List<Countries> countries = await System.Threading.Tasks.Task.Run(() => NearByMeManager.GetInstance().GetNBMMembersCountryList());
                return Request.CreateResponse(HttpStatusCode.OK, new { returnValue= countries });
            }
            catch (ApplicationException applicationException)
            {
                return Request.CreateErrorResponse((HttpStatusCode)Convert.ToInt16(applicationException.Message), NeeoDictionaries.HttpStatusCodeDescriptionMapper[Convert.ToInt16(applicationException.Message)]);
